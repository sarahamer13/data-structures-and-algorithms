...
const sortByChildren = (charArray) => {
  return charArray.sort ((a,b) => {
    return a.children.length < b.children.length ? -1 : a.children.length > b.children ? 1: -1;
  });
};

// using ternary here. If a children is shorter than b children then function will be returned as -1 and a will come before b
// if b children is longer than a children then function will return 1 and b will come before a
// if a same as b children then function will return -1
...
// checking if letter w is in the string
//.test is a method to call on the regular expression.
...
// using \d to find digit characters
...
//using \b to find exact word match
...
const isCapitalized = (str) => {
  const data = str.match(/[A-Z][a-z]*/g);
  return data || [];
};

//finding all words that are capitalized and return them as an array
// using [A-Z] to find any matches with uppercase letters
//using [a-z]* to find matches with a word that has one capital letter or all capital or followed by zero
//using g flag which means global to scoop to words with capital letters.
//finally returning either the actual match or || empty array if it's false
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const citiesAtoJ = (arr) => {
  return arr.filter ((value) => /^[A-J]/g.test(value));
};

//using filter and assigning value as parameter
//using ^[A-J] to find any match between A through J
// using g flag again and then test method to check if it's true or false.
// if it's true, the outcome will be filtered to include only the elements.
