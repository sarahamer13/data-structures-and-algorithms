...
const screenForNames = (arr) => {
  const data = /^(Mr.|Mrs.|Ms.|Dr.) [a-zA-Z]+/;
  return arr.filter(value => data.test(value));
};
...
let biggerThanLuke = (arr) => {
  const data = arr.filter(value => parseInt(value.mass) > parseInt(arr[0].mass))
...
const detectTicTacToeWin = (board) => {
  function helpCheck(row1, co11, row2, co12, row3, col13) {
    if (
      board[row1][co11] === board[row2][co12] &&
      board[row2][co12] === board[row3][col13] &&
      board[row1][co11] !== ' '
    ) {
      return true;
    }
...
// const data = [...board];
// const data2 = data.concat(data.reduce((acc, v) => {
//   return v.map((val,i) => acc[i].concat([val]));
// }, [[],[],[]]))
//   .concat([[data[0][0],data[1][1],data[2][2]]])
//   .concat([[data[0][2],data][1],data][0]);
// return data2.some(v => v.every(val => val === 'X') || v.every(val => val === 'O'));
