...
const addExclamation = (arr) => {
  const data = [];
  arr.forEach (value => {
    data.push (value + '!'); // for each element in arr the function concatenates the string '!' to it by using + then push the resulted string
  });
  return data;
};
...
const allUpperCase = (arr) => {
  const data = [];
  arr.forEach (value => {  // loop over each element of the input array then the loop passes each element to the arrow function inside the parentheses
    data.push (value.toUpperCase ());  // convert to UC then adds it to the data array using push
  });
  return data;
};
...
const greeting = (word) => {
  ...
  newWord += '!';                // appends an exclamation to the end of the newWord
  return newWord;
};

const speaker = (words, callback) => {
  let data = [];
  words.forEach (value => {  // iterate over each value in the words array then the function calls the callback function passing in the value as the input
    data.push (callback(value));    // This is the resulted value returned by the callback, pushed into the data array.
  });
  return data;
};
